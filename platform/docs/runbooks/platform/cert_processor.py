#!/usr/bin/env python3
"""platform/cert_processor.py - Phase 3: Certificate processing (chain build, PFX)."""
...
class CertificateProcessor:
    ...
    def build_chain(self, cert_path, intermediate_path="", root_path="", output_path=""):
        ...
        self.log.info("Chain built: %s (%d certs)", output_path, count)
        return output_path

    def build_pfx(self, cert_path, key_path, output_path="", password=""):
        if not self.openssl: return {"success":False,"error":"OpenSSL not found"}
        if not output_path: output_path = os.path.splitext(cert_path)[0] + ".pfx"
        pw = f"pass:{password}"
    ...
    def validate_certificate(self, cert_path):
        if not self.openssl: return {"valid":False,"error":"OpenSSL not found"}
