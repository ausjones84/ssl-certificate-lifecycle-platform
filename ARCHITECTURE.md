...
1. Self-contained — no external repository dependencies
2. Approval-gated — no production changes without explicit confirmation
3. Dry-run first — all write operations support dry-run simulation
4. Audit trail — every operation logged to timestamped files
5. Multi-format output — all reports generated in CSV, JSON, MD, HTML
6. Zero tribal knowledge — complete documentation for any engineer
...
## Layer Architecture
    ...
    Orchestration Layer
        ...
        |- cert_processor.py Phase 3: Chain/PFX
...
The platform can be run in three modes:

1. Interactive CLI: python platform/main.py [command]
2. PowerShell Scripts: scripts/*/[Script].ps1
3. Azure DevOps Pipeline: pipelines/azure-pipelines-cert-renewal.yml

All three modes share the same underlying logic and produce the same outputs.

## Scalability

The platform scales to multiple subscriptions, vaults, and app services by:
- Iterating over subscriptions in config/environments.json
- Using Azure CLI context switching between subscriptions
- Batch discovery across all resource groups
